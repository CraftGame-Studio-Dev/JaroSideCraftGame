...
public class TextureAtlas {
	...
	public static Texture transparent1x1() {
		TextureAtlas text=new TextureAtlas(GL_TEXTURE_2D,1,1);
		int[] data= {0};//Hard coding
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
		return new Texture(text,0,0,1,1);
	}
	public static Texture error() {
		TextureAtlas text = new TextureAtlas(GL_TEXTURE_2D,16,16);
	...
	public static TextureAtlas sky() {
		TextureAtlas text = new TextureAtlas(GL_TEXTURE_CUBE_MAP,2,2);
		int data[] = {0xFFF800F8,0xFF000000,0xFF000000,0xFFF800F8};//Hard coding
		glGenerateMipmap(GL_TEXTURE_2D);
	...
	public static Texture create(InputStream in) {
		Texture text;
		try {
			text=new Texture(new TextureAtlas(in),0,0,0,0);
		} catch (Exception e) {
	...
	public static Texture create(byte[] data) {
		Texture text;
		try {
			text=new Texture(new TextureAtlas(data),0,0,0,0);
		}catch(Exception e) {
			System.err.println("[ERROR] Invalid Texture");
			text=Constant.INVALID_TEXTURE_HARD_CODING;
		}
